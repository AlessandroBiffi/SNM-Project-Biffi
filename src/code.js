...
function viewPlaylistsPersonali() {
  ...
  fetch("http://127.0.0.1:" + port + "/users/playlistPersonali/view?id=" + myID, {
    ...
    .then(res => {
      ...
      if (res.length == 0) {
        document.getElementsByClassName("loadingBox")[0].classList.remove("d-none")
        document.getElementById("loadPersonali").innerHTML = "non hai creato ancora nessuna playlist"
        document.getElementById("loadPersonali").classList.remove("loading")
        document.getElementById("loadPersonali").style.opacity = 0.2
      }

    })
}

//VISTA PLAYLIST IMPORTATE E SEGUITE

function viewPlaylistsImportateSeguite() {

  var myID = sessionStorage.getItem("_iduser")

  fetch("http://127.0.0.1:" + port + "/users/playlistPersonali/viewLikedFollow?id=" + myID, {
      method: 'GET',
      headers: {
        "Content-Type": "application/json"
      }
    ...
    .then(res => {
      ...
      for (var i = 0; i < res.itemImp.length; i++) {

        var playlist = document.getElementsByClassName("singola-playlist-imported")[0]
        var clone = playlist.cloneNode(true)
        clone.classList.remove('d-none')
        clone.getElementsByClassName("titolo-playlist")[0].innerHTML = res.itemImp[i].nome
        clone.getElementsByClassName("tipo-playlist")[0].innerHTML = res.itemImp[i].tipo
        clone.getElementsByClassName("data-playlist")[0].innerHTML = res.itemImp[i].data_creazione
        clone.getElementsByClassName("vedi-playlist")[0].id = res.itemImp[i]._id
        clone.getElementsByClassName("del-playlist")[0].id = res.itemImp[i]._id
        playlist.before(clone)

      }
      document.getElementsByClassName("loadingBox")[1].classList.add("d-none")

      if (res.itemImp.length == 0) {
        document.getElementsByClassName("loadingBox")[1].classList.remove("d-none")
        document.getElementById("loadImportate").innerHTML = "nessuna playlist importata"
        document.getElementById("loadImportate").classList.remove("loading")
        document.getElementById("loadImportate").style.opacity = 0.2
      }

      for (var i = 0; i < res.data_itemFollow.length; i++) {

        var playlist = document.getElementsByClassName("singola-playlist-seguite")[0]
        var clone = playlist.cloneNode(true)
        clone.classList.remove('d-none')
        clone.getElementsByClassName("titolo-playlist")[0].innerHTML = res.data_itemFollow[i].nome
        clone.getElementsByClassName("tipo-playlist")[0].innerHTML = res.data_itemFollow[i].tipo
        clone.getElementsByClassName("data-playlist")[0].innerHTML = res.data_itemFollow[i].data_creazione
        clone.getElementsByClassName("vedi-playlist")[0].id = res.data_itemFollow[i]._id
        clone.getElementsByClassName("del-playlist")[0].id = res.data_itemFollow[i]._id
        playlist.before(clone)
      ...
      if (res.data_itemFollow.length == 0) {
        document.getElementsByClassName("loadingBox")[0].classList.remove("d-none")
        document.getElementById("loadSeguite").innerHTML = "nessuna playlist seguita"
        document.getElementById("loadSeguite").classList.remove("loading")
        document.getElementById("loadSeguite").style.opacity = 0.2
      }
...
function trackView() {
  ...
  fetch("https://api.spotify.com/v1/tracks/" + idTrack, {
    ...
    .then((trackInfo) => {
      ...
      document.getElementById("boxLoading").classList.add("d-none")
...
function selectTrackPlaylist() {
  ...
  fetch("http://127.0.0.1:" + port + "/users/playlistPersonali/view?id=" + myID, {
    ...
    .then(res => {
      ...
      var checkOnlyImported = 1
      for (var i = 0; i < res.length; i++) {
        if (res[i].tipo != 'importata')
          checkOnlyImported = 0 //cosi capisco se sono tutte importate e deve apparire ancora "crea playlist"
      }
...
function viewPlaylistPersonale(idplaylist) {
  ...
  fetch("http://127.0.0.1:" + port + "/users/playlistPersonali/viewOne?idplaylist=" + idplaylist, {
    ...
    .then(res => {
      ...
      sessionStorage.setItem("tipoPlaylist", res.tipo)
      document.getElementById("box1Loading").classList.add("d-none")
      document.getElementById("box3Loading").classList.add("d-none")
      document.getElementById("box4Loading").classList.add("d-none")
      ...
      //inizializzazioni bottoni back

      var currentIdUser = sessionStorage.getItem("_iduser")
      if (res._iduser == currentIdUser && !res._idOriginale) {
        document.getElementById("setURLCorrettoBack").onclick = function() {
          location.href = 'personalplaylist.html'
        }
      } else {
        document.getElementById("setURLCorrettoBack").onclick = function() { //se mi ritrovo a guardare una playlist importata o salvata tornero allla loro pagina dedicata poi e non a quella delle mie playlist personali
          location.href = 'importate_seguite.html'
        }
      }

      if (!res.importFrom) { //se e' una playlist con like e non importata  almeno non ci sono fastidi
      ...
      //check playlist se e' una seguita, in casso imposto tipoPlaylist = like e tolgo mia o importata

      var currentNameUser = sessionStorage.getItem("name_user")
      if (res.liked_by) {
        for (var i = 0; i < res.liked_by.length; i++) {
          if (res.liked_by[i] == currentNameUser) {
            sessionStorage.setItem("idBy", res._iduser)
            sessionStorage.removeItem("tipoPlaylist")
            sessionStorage.setItem("tipoPlaylist", "like")
          }
        }
      }

      seeTracksPlaylist(res.songs) //tipo uso per non permettere di eliminare song da canzoni importate
      ...
      if (idBy != "(account eliminato)") {
        ...
        fetch("http://127.0.0.1:" + port + "/user/" + idBy, {
          ...
          .then(res => {
            ...
            if (sessionStorage.getItem("tipoPlaylist") == "like") {
              document.getElementById("byIf").innerHTML = "Creata da " + res.name_user
              document.getElementById("box2Loading").classList.add("d-none")
            } else if (sessionStorage.getItem("tipoPlaylist") == "importata") {
...
function seeTracksPlaylist(songs) {
  ...
  for (var i = 0; i < songs.length; i++) {
    ...
    fetch("https://api.spotify.com/v1/tracks/" + songs[i], {
      ...
      .then(res => {
        ...
        if (sessionStorage.getItem("tipoPlaylist") !== "importata" && sessionStorage.getItem("tipoPlaylist") !== "like") {
...
//MOSTRA ULTIME 10 PLAYLIST PUBBLICHE CREATE
...
function viewPlaylistPubblica() {
  ...
  fetch("http://127.0.0.1:" + port + "/esplora/playlistPubblica?idplaylist=" + idplaylist, {
    ...
    .then(res => {
      ...
      document.getElementById("titolo-playlist").innerHTML = res.nome
      document.getElementById("descrizione-playlist").innerHTML = res.descrizione
      sessionStorage.setItem("_idPlayPub", res._id)
      for (var i = 0, allTags = ''; i < res.tags.length; i++) {
        allTags = allTags + " " + res.tags[i]
      }
      document.getElementById("tags-playlist").innerHTML = allTags
      ...
      isLiked() //qui interni permette di settare prima _idPlayPub per non commettere errori di cuore pieno vuoto
    })
...
//MOSTRA CUORE PIENO O VUOTO A SECONDA SE SIA GIA STATO MESSO LIKE O NO ALLA PLAYLIST

function isLiked() {
  var iduser = sessionStorage.getItem("_iduser")
  var idPlaylistPub = sessionStorage.getItem("_idPlayPub")

  var data = {
    "_iduser": iduser,
    "idplaylistPub": idPlaylistPub
  }

  fetch("http://127.0.0.1:" + port + "/users/playlistPersonali/isLiked", {
      method: 'POST',
      headers: {
        "Content-Type": "application/json"
      },
    ...
    .then(res => {
      ...
      console.log(res)
      document.getElementById("numLike").innerHTML = res.numLike
      if (res.status == '0') { //no like
        document.getElementsByClassName("like-song")[0].src = "../../styles/icons/cuore-vuoto.png"
        document.getElementsByClassName("like-song")[0].name = "like"
      } else if (res.status == '1') {
        document.getElementsByClassName("like-song")[0].src = "../../styles/icons/cuore-pieno.png"
        document.getElementsByClassName("like-song")[0].name = "unlike"
      }
...
//IMPORTA PLAYLIST NEL PROPRIO PROFILO
...
//AGGIUNTA O RIMOZIONE PLAYLIST AI SEGUITI

function addUnaddSeguiti(tipoAzione) {
  ...
  fetch("http://127.0.0.1:" + port + "/users/playlistPersonali/typelike", {
      method: 'POST',
      headers: {
        "Content-Type": "application/json"
      },
...
//RIMOZIONE SINGOLA PLAYLIST DAI SEGUITI PER MENU Importate_seguite

function unaddSeguito(idPlayLike) {

  var iduser = sessionStorage.getItem("_iduser")

  var data = {
    "_iduser": iduser,
    "idplaylistPub": idPlayLike,
    "azione": "unlike"
  }
  ...
  fetch("http://127.0.0.1:" + port + "/users/playlistPersonali/typelike", {
    ...
    .then(res => {
      ...
      if (res.acknowledged == true) {
        alert("hai smesso di seguire la playlist")
        window.location.reload()
      } else {
        alert(res)
      }
